...
namespace SidebarDiagnostics
{
    ...
    public sealed class StoredSettings : INotifyPropertyChanged
    {
        ...
        {
            StoredSettings _return = null;

            if (File.Exists(Paths.SettingsFile))
            {
                using (StreamReader _reader = File.OpenText(Paths.SettingsFile))
                {
                    _return = (StoredSettings)new JsonSerializer().Deserialize(_reader, typeof(StoredSettings));
                }
            }

            return _return ?? new StoredSettings();
        }
        ...
        private static StoredSettings _instance { get; set; } = null;

        public static StoredSettings Instance
        {
            get
            {
                if (_instance == null)
