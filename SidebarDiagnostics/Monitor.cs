...
namespace SidebarDiagnostics.Monitor
{
    public class MonitorManager
    {
        ...
        public void AddPanel(MonitorConfig config)
        {
            switch (config.Type)
            {
                case MonitorType.CPU:
                    OHMPanel(
                        config.Type,
                        (bool)config.Params.Single(p => p.Key == MonitorConfigParam.Defaults.HardwareNames.Key).Value,
        ...
        private void OHMPanel(MonitorType type, bool showNames, string pathData, params HardwareType[] hardwareTypes)
        {
            ...
            {
                OHMMonitor _monitor = new OHMMonitor(type, _hardware, _board, showNames);
    ...
    public class OHMMonitor : iMonitor
    {
        ...
        private void InitCPU(IHardware board)
        {
            ...
            if (_voltage != null)
            {
                _sensorList.Add(new OHMSensor(_voltage, "Volt", " V"));
            }

            if (_tempSensor != null)
            {
                _sensorList.Add(new OHMSensor(_tempSensor, "Temp", " C"));
            }

            if (_fanSensor != null)
            {
                _sensorList.Add(new OHMSensor(_fanSensor, "Fan", " RPM"));
            }

            List<ISensor> _loadSensors = _hardware.Sensors.Where(s => s.SensorType == SensorType.Load).ToList();
        ...
        public void InitRAM(IHardware board)
        {
            ...
            if (_loadSensor != null)
            {
                _sensorList.Add(new OHMSensor(_loadSensor, "Load", "%"));
            }
            ...
            if (_usedSensor != null)
            {
                _sensorList.Add(new OHMSensor(_usedSensor, "Used", " GB"));
            }
            ...
            {
                _sensorList.Add(new OHMSensor(_availSensor, "Free", " GB"));
            }
        ...
        public void InitGPU()
        {
            ...
            if (_coreLoad != null)
            {
                _sensorList.Add(new OHMSensor(_coreLoad, "Core", "%"));
            }
            ...
            if (_memoryLoad != null)
            {
                _sensorList.Add(new OHMSensor(_memoryLoad, "VRAM", "%"));
            }
            ...
            if (_fanSensor != null)
            {
                _sensorList.Add(new OHMSensor(_fanSensor, "Fan", "%"));
            }
