...
namespace SidebarDiagnostics.Monitor
{
    public class MonitorManager : INotifyPropertyChanged, IDisposable
    {
        public MonitorManager(MonitorConfig[] config)
        {
            ...
            MonitorPanels = config.Where(c => c.Enabled).OrderBy(c => c.Order).Select(c => AddPanel(c)).ToArray();
        }
        ...
        public MonitorPanel AddPanel(MonitorConfig config)
        {
            switch (config.Type)
            {
                ...
                default:
                    throw new ArgumentException("Invalid MonitorType for config parameter.");
            }
        }
