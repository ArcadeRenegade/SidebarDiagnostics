...
namespace SidebarDiagnostics.Monitoring
{
    public class MonitorManager : INotifyPropertyChanged, IDisposable
    {
        ...
        private MonitorPanel OHMPanel(MonitorType type, string pathData, string[] disabledHardware, ConfigParam[] parameters, params HardwareType[] hardwareTypes)
        {
            return new MonitorPanel(
                type.GetDescription(),
                pathData,
                GetHardware(hardwareTypes).Where(h => !disabledHardware.Contains(h.Identifier.ToString())).Select(h => new OHMMonitor(type, h, _board, parameters)).ToArray()
                );
        }

        private MonitorPanel DrivePanel(MonitorType type, string[] disabledHardware, ConfigParam[] parameters)
        {
            return new MonitorPanel(
                type.GetDescription(),
                ...
                new DriveMonitor(disabledHardware, parameters)
                );
        }

        private MonitorPanel NetworkPanel(MonitorType type, string[] disabledHardware, ConfigParam[] parameters)
        {
            return new MonitorPanel(
                type.GetDescription(),
                ...
                new NetworkMonitor(disabledHardware, parameters)
                );
        }
    ...
    public class MonitorConfig : INotifyPropertyChanged, ICloneable
    {
        ...
        public string[] DisabledHardware
        {
            get
            {
                if (Hardware == null)
                ...
                return Hardware.Where(h => !h.Enabled).Select(h => h.ID).ToArray();
            }
        }
